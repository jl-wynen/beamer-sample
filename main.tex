...
\begin{frame}[fragile]\frametitle{Shell commands and files}
  ...
  \begin{itemize}
  \item First thing:
    % select shell language and style manually
    \begin{lstlisting}[language=aiphishell, style=aiphishell]
mkdir <your-dir>
cd <your-dir>
    \end{lstlisting}
  \item Then:
    % set language and style 'automatically'
    \begin{aiphishell}
git add <a-file>
...
\begin{frame}\frametitle{Directory Trees (or directory-like files)}
  ...
  for tree={
    ...
    calign=first,
    edge path={
      \noexpand\path [draw, \forestoption{edge}, black!70!white]
      ([xshift=7.5pt]!u.south west) |- node[fill,inner sep=1.25pt] {} (.child anchor)\forestoption{edge label};
    },
  ...
  [{\color{aiphired}.h5}
    ...
    [{\color{aiphipetrol}configuration}
      ...
      [\ldots, edge path={
        \noexpand\path [draw, \forestoption{edge}, dashed, black]
