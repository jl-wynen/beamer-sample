\documentclass[mathserif, fleqn, aspectratio=169]{beamer}
\input{settings.tex}
...
\begin{frame}\frametitle{Stuff}
  ...
  \item Emphasis \empha{\textbackslash{}empha} \emphb{\textbackslash{}emphb} \emphc{\textbackslash{}emphc} \emph{\textbackslash{}emph}
  \end{itemize}
\end{frame}
      ...
      \begin{column}{.4\framewidth}
        \begin{center}
          \begin{tikzpicture}
            ...
            \draw (n0) -- node [rectangle, fill=aiphibackground, text=aiphiblue!70!white] {1} (n1) [draw=aiphiblue!70!white, line width=.3mm];
            \draw (n0) -- node [rectangle, fill=aiphibackground, text=aiphiblue!70!white] {2} (n4) [draw=aiphiblue!70!white, line width=.3mm];
            \draw (n3) -- node [rectangle, fill=aiphibackground, text=aiphiblue!70!white] {2} (n4) [draw=aiphiblue!70!white, line width=.3mm];
            \draw (n0) -- node [rectangle, fill=aiphibackground, text=aiphiblue!70!white] {2} (n3) [draw=aiphiblue!70!white, line width=.3mm];
            \draw (n1) -- node [rectangle, fill=aiphibackground, text=aiphiblue!70!white] {1} (n2) [draw=aiphiblue!70!white, line width=.3mm];
            \draw (n2) -- node [rectangle, fill=aiphibackground, text=aiphiblue!70!white] {1} (n3) [draw=aiphiblue!70!white, line width=.3mm];
          \end{tikzpicture}
        \end{center}
      \end{column}
    \end{columns}
...
struct foreach {
  ...
  static void f(Args && ...args) {
    ...
    char const c = 'c';
    std::vector<int> v((*\fparam{n}*)3);  // what is the length of this?
  }
};
  \end{cppcode}
\end{frame}
...
isingmc (cfg) -> E, m:
    (*\emphc{E $\leftarrow$ hamiltonian(E)}*)

    repeat ntherm:
        cfg, (*\emphc{E}*) <- sweep(cfg, (*\emphc{E}*))

    repeat nprod:
        cfg, (*\emphc{E}*) <- sweep(cfg, (*\emphc{E}*))
        (*\empha{E $\leftarrow$ hamiltonian(cfg)}*)
        m <- magnetisation(cfg)
...
\begin{frame}{Boxes in itemize}
  \begin{itemize}
  \item An item without box
    \boxeditem{aiphiblue}{}{%
    \item An item with default box
    }
    \boxeditem{aiphiyellow}{right=-70mm}{%
    \item An item with a reduced box width
    }
  \end{itemize}
  Note that \textbackslash{}setlength\{\textbackslash{}itemsep\} does not work properly.
\end{frame}

{
  \usebackgroundtemplate{\begin{tikzpicture}
      \clip (0, 0) rectangle (\paperwidth, \paperheight);
      \fill[fill=aiphibackground] (0, 0) rectangle (\paperwidth, \paperheight);
...
\begin{frame}\frametitle{Arrows}
  ...
  \begin{tikzpicture}[remember picture, overlay]
    ...
    in
      ...
      node[shape=circle, fill=aiphitext, inner sep=1pt] {};
...
\begin{frame}{TikZ Particles}
  \begin{itemize}\setlength\itemsep{2em}
  \item Baryons:
    \begin{tikzpicture}
      \tikzstyle{quark}=[circle, draw=none, inner sep=0pt, minimum size=1.25mm];
      \tikzstyle{baryon}=[circle, draw=aiphitext!50!aiphibackground, fill=none, inner sep=0pt, minimum size=3mm, append after command={
        +(0,0.75mm) node [quark, fill=aiphired] {}
        +(0.65mm,-0.375mm) node [quark, fill=aiphigreen] {}
        +(-0.65mm,-0.375mm) node [quark, fill=aiphiblue] {}
      }];

      \node [baryon] at (0, 0) {};
    \end{tikzpicture}
  \item Gluons (needs `snakes' TikZ library):
    \begin{tikzpicture}
      % this needs a bit of fine tuning to get the ends right
      \tikzstyle{gluon}=[snake=coil, segment length=1mm];
      \draw[gluon] (0,0) -- (9.4mm,0);
    \end{tikzpicture}
  \item Photons (also needs `snakes'):
    \begin{tikzpicture}
      % this needs a bit of fine tuning to get the ends right
      \tikzstyle{photon}=[snake=coil, segment aspect=0];
      \draw[photon] (0,0) -- (8.8mm,0);
    \end{tikzpicture}
  \end{itemize}
\end{frame}
...
\begin{frame}\frametitle{Custom aiphi style for Matplotlib}
  \begin{center}
    \begin{enumerate}
    \item Copy \textdv{\small extra/aiphidark.mplstyle} into your plotting directory
    \item Use the style by, e.g.\ \textdv{\small plt.style.use('aiphidark.mplstyle')}
    \end{enumerate}
    \vfill
...
\begin{frame}[plain]
  ...
  \begin{textblock*}{\paperwidth} (0cm, 0cm)
    \begin{tikzpicture}
      \clip (0, 0) rectangle (\paperwidth, \paperheight);
      % fill the page with the background colour
      \fill[fill=aiphibackground] (0, 0) rectangle (\paperwidth, \paperheight);

      % ------ insert background graphic here if you want one ------

      % draw rectangle in centre
      \fill[fill=aiphibackground!50!black, opacity=0.7] (0, 0.33\paperheight) rectangle (\paperwidth, .66\paperheight);

      % draw lines on centre part
      \foreach \y in {0.33, 0.34, ..., 0.66}
      \draw[draw=aiphibackground!85!black] (0, \y\paperheight) -- (\paperwidth, \y\paperheight);

      % Text
      \node at (0.5\paperwidth, 0.5\paperheight) [align=center] {\Huge\usebeamercolor[fg]{frametitle}Backup Slides};
    \end{tikzpicture}
  \end{textblock*}
\end{frame}
